...
class IntroSocial extends WP_Widget{
	...
	public function __construct(){
		parent::__construct(
			'IntroSocial',
			__('Gavroche - Social', 'gavroche'),
			array('description'=>__('Display links to your social profiles.', 'gavroche'),)
		);
	}
		...
		foreach ($fields as $count=>$field){
			...
			<p>
				<label for="<?php echo $this->get_field_id($field); ?>">
					<?php echo ($field == 'title' ? __('Title', 'gavroche') : ucfirst(str_replace("-"," ",$field)) ) . ':'; ?>
				</label> 
 ...
 * @return void
 */
if (!function_exists('gavroche_widgets_init')){
	function gavroche_widgets_init(){
		register_widget('IntroSocial');
	}
}
add_action('widgets_init', 'gavroche_widgets_init');